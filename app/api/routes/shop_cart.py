...
from app.models.tables import CartItem, Sku, Product
...
async def get_cart(request: Request, db: Session = Depends(get_db)):
    ...
    result = []
    for item in items:
        sku = db.execute(select(Sku).where(Sku.id == item.sku_id)).scalar_one_or_none()
        product = db.execute(select(Product).where(Product.id == sku.product_id)).scalar_one_or_none() if sku else None
...
async def add_to_cart(body: AddCartBody, request: Request, db: Session = Depends(get_db)):
    """添加商品到购物车"""
    uid = await _uid(request)

    sku = db.execute(select(Sku).where(Sku.id == body.sku_id)).scalar_one_or_none()
    if not sku:
    ...
    try:
        ...
        record(db, uid, sku.product_id, "cart")
    except Exception:
        pass
...
async def select_all(request: Request, db: Session = Depends(get_db)):
    """切换全选状态"""
    uid = await _uid(request)
    body = await request.json()
    selected = bool(body.get("selected", True))
    db.execute(update(CartItem).where(CartItem.user_id == uid).values(selected=selected))
    db.commit()
    return {"code": 0, "message": "ok"}


@router.put("/{item_id}/select", summary="选中某条")
async def select_one(item_id: int, request: Request, db: Session = Depends(get_db)):
    """切换单条选中状态"""
    uid = await _uid(request)
    body = await request.json()
    selected = bool(body.get("selected", True))
