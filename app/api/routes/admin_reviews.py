...
async def list_reviews(
    ...
    data, total = ReviewService.list_reviews(db, page, page_size, status, product_id)
    return {"code": 0, "data": data, "total": total, "page": page, "page_size": page_size}


@router.put("/{review_id}/approve", summary="通过评价")
async def approve_review(review_id: int, user: dict = Depends(require_permission("reviews", "update")), db: Session = Depends(get_db)):
    try:
        result = ReviewService.approve_review(db, review_id, True)
...
async def reject_review(review_id: int, user: dict = Depends(require_permission("reviews", "update")), db: Session = Depends(get_db)):
    try:
        result = ReviewService.approve_review(db, review_id, False)
...
async def reply_review(
    ...
    try:
        result = ReviewService.reply_review(db, review_id, body.reply)
