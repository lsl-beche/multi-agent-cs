...
async def list_reviews(
    ...
    data, total = await run_sync(db, ReviewService.list_reviews, page, page_size, status, product_id)
...
async def approve_review(review_id: int, user: dict = Depends(require_permission("reviews", "update")), db: AsyncSession = Depends(get_db)):
    try:
        result = await run_sync(db, ReviewService.approve_review, review_id, True)
...
async def reject_review(review_id: int, user: dict = Depends(require_permission("reviews", "update")), db: AsyncSession = Depends(get_db)):
    try:
        result = await run_sync(db, ReviewService.approve_review, review_id, False)
...
async def reply_review(
    ...
    try:
        result = await run_sync(db, ReviewService.reply_review, review_id, body.reply)
