...
| **2000 并发 Locust 零超卖**(库存500,Hikari pool=80) | 恰好 **500×200 / 1500×409 / 终态 0 / 0 失败**,守恒律 `成功×数量==初始−终态` PASS(`scripts/oversell_load_test.py`) |
| 2000 并发(默认 pool=10 / conn-timeout 3s) | 约 81% 请求因连接池耗尽返 **500**,但**仍零超卖**——库存未转负、扣减量恒等于成功数(饱和是容量问题,非一致性缺陷) |
| 对拍:20 并发确认 | 原子版副作用恰好 1 次;legacy(复刻 Python 缺陷)20 次 |
| 订单全生命周期(创建→支付→取消) | 集成测试 5 用例 PASS |
| 支付标记金额校验 | 不一致 40906 拒绝,重放幂等 |
| 集成测试(Testcontainers/Compose MySQL) | **30/30 PASS** |
| 越权防护单测(归属匹配/不匹配/游客/空订单) | 5/5 PASS |
| **3000 笔/40 线程容量压测**(60 身份轮转) | **100% 200、零超卖**、吞吐 37 req/s、P50 1030ms / P99 1822ms |
| **10000 笔/40 线程容量压测**(Session 连接复用) | **100% 200、零超卖**、吞吐 40 req/s、P50 977ms / P99 1519ms |

> 容量调优记录:Hikari 连接池 10→20、获取超时 3s→10s——压测发现 P99 已达旧超时阈值 60%,
> 调优后消除高并发连接超时尾部风险;吞吐天花板由热点单行行锁串行化决定(37 req/s),
> 与连接池无关(A/B 实测延迟持平)。高并发秒杀场景的演进路径:Redis 预扣减 + MQ 异步落单,
> 以强一致换吞吐(当前量级不需要)。
